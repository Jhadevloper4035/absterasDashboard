...
export function leaveDays(fromDate, toDate, holidayDates = []) {
  const from = dayAtMidnight(fromDate);
  const to = dayAtMidnight(toDate);
  if (!from || !to || to < from) return 0;
  const holidays = new Set(holidayDates.map((date) => new Date(date).toISOString().slice(0, 10)));
  // ponytail: calendar days until the company approves a workweek policy; add a configured workweek here then.
  let days = 0;
