...
const line = new mongoose.Schema({
  inventoryItemRef: { type: ObjectId, required: true }, itemName: { type: String, required: true, trim: true }, hsnCode: { type: String, trim: true }, unit: { type: String, trim: true }, quantity: { type: Number, required: true, min: 0.01 },
...
const order = new mongoose.Schema({
  ...
  items: { type: [line], validate: [(items) => items.length > 0, 'At least one product is required'] }, status: { type: String, enum: ['OUT', 'RETURNED'], default: 'OUT' }, outwardChallanRef: ObjectId, inwardChallanRef: ObjectId, createdBy: { type: ObjectId, required: true },
}, { timestamps: true });
