...
const EmployeeOverviewPage = () => {
  ...
  return <>
    ...
    {overview && <>
      <Card className="mb-4 border-start border-4 border-primary shadow-sm"><CardBody className="d-flex justify-content-between align-items-center flex-wrap gap-3"><div><div className="text-muted small mb-1">Viewing employee</div><h5 className="mb-1">{overview.employee.user.name}</h5><p className="text-muted mb-0">{overview.employee.department?.name || 'No department'} · {overview.employee.designation?.name || 'No job title'}</p></div>{isMine && <div className="d-flex gap-2 flex-wrap"><Link to="/hr/my-leave"><Button size="sm" variant="outline-primary">Request leave</Button></Link><Link to="/hr/expenses"><Button size="sm" variant="outline-primary">Claim reimbursement</Button></Link><Link to="/hr/advances"><Button size="sm" variant="outline-primary">Request advance</Button></Link></div>}</CardBody></Card>
