...
const ReportsPage = () => {
  ...
  return <><PageMetaData title={selectedEmployee ? `${selectedEmployee.user.name} attendance report` : 'Attendance report'} /><Card><CardBody><div className="d-flex justify-content-between align-items-start gap-3 mb-3"><div><h4 className="card-title mb-1">Attendance report</h4><p className="text-muted mb-0">View the selected employee’s attendance for the current month or a custom date range.</p></div><PdfActionButton disabled={!employee} variant="outline-primary" action={() => printPdf(`/hr/attendance/report.pdf?employee=${employee}&from=${from}&to=${to}`, token).catch((reason) => setError(reason instanceof Error ? reason.message : 'Unable to open attendance report PDF'))}>Print PDF</PdfActionButton></div>{error && <Alert variant="danger">{error}</Alert>}<div className="d-flex gap-2 flex-wrap mb-3"><Form.Select value={employee} onChange={(event) => setEmployee(event.target.value)} style={{ width: 260 }}><option value="">Select employee</option>{employees.map((item) => <option key={item._id} value={item._id}>{item.user.name} · {item.department?.name || 'No department'}</option>)}</Form.Select><Form.Control aria-label="From date" type="date" value={from} onChange={(event) => setFrom(event.target.value)} style={{ width: 180 }} /><Form.Control aria-label="To date" type="date" value={to} onChange={(event) => setTo(event.target.value)} style={{ width: 180 }} /></div><div className="attendance-report-table"><ReactTable columns={columns} data={records} pageSize={25} tableClass="text-nowrap mb-0" theadClass="bg-light bg-opacity-50" /></div></CardBody></Card></>
}
