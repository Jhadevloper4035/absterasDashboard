...
import { AttendanceReportCard } from './reports/page'
...
type Record = { _id: string; date: string; status: string; checkIn?: string; checkOut?: string; workMinutes?: number; isShortLeave?: boolean; overtimeMinutes: number; employee: EmployeeType; markedBy?: { name: string }; correctionRequest?: { status: string; reason: string; requestedStatus: string; requestedCheckIn?: string; requestedCheckOut?: string } }
...
const AttendancePage = () => {
  ...
  const canApprove = canApproveHrRequests(user)
  ...
  const save = async (event: FormEvent) => {
    ...
    if (date > today()) return setError('Attendance can only be marked for today or a previous day')
  ...
  const decideCorrection = async (record: Record, status: 'approved' | 'rejected') => {
    const approving = status === 'approved'
    const result = await Swal.fire({
      icon: approving ? 'question' : 'warning',
      title: `${approving ? 'Approve' : 'Reject'} attendance correction?`,
      text: approving ? `Apply the requested correction for ${record.employee?.user?.name || 'this employee'}?` : 'The employee will be notified that this correction was rejected.',
      showCancelButton: true,
      confirmButtonText: approving ? 'Approve correction' : 'Reject correction',
      confirmButtonColor: approving ? undefined : '#dc3545',
    })
    if (!result.isConfirmed) return
    setSaving(true); setError('')
    try { await apiFetch(`/hr/attendance/${record._id}/correction`, { method: 'PATCH', body: JSON.stringify({ status }) }); toast.success(`Correction ${status}`); await load() } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to decide correction') } finally { setSaving(false) }
  }
  ...
  const columns = useMemo<ColumnDef<Record>[]>(() => [
    ...
    { header: 'Correction', cell: ({ row }) => { const request = row.original.correctionRequest; return request ? <div><Badge bg={request.status === 'pending' ? 'warning' : request.status === 'approved' ? 'success' : 'danger'} text={request.status === 'pending' ? 'dark' : undefined}>{request.status}</Badge><small className="d-block text-muted">Requested: {request.requestedStatus}{request.requestedCheckIn ? ` · ${request.requestedCheckIn}` : ''}{request.requestedCheckOut ? `–${request.requestedCheckOut}` : ''}<br />{request.reason}</small>{canApprove && request.status === 'pending' && <div className="d-flex gap-1 mt-1"><Button size="sm" disabled={saving} onClick={() => decideCorrection(row.original, 'approved')}>Approve & correct</Button><Button size="sm" variant="outline-danger" disabled={saving} onClick={() => decideCorrection(row.original, 'rejected')}>Reject</Button></div>}</div> : '—' } },
    { header: 'Marked by', cell: ({ row }) => row.original.markedBy?.name || '—' },
  ], [canApprove, saving])

  return <><PageMetaData title="Attendance" /><Card className="mb-4"><CardBody><div className="d-flex justify-content-between align-items-start gap-3 mb-4"><div><h4 className="card-title mb-1">Mark attendance</h4><p className="text-muted mb-0">Late after 10:30 AM is flagged. Arrival after 4:00 PM is a half-day salary deduction.</p></div><Badge bg="info" className="px-3 py-2">HR manager only</Badge></div>{error && <Alert variant="danger">{error}</Alert>}<Form onSubmit={save}><Row className="g-3"><Col lg={2} md={4}><Form.Label>Date</Form.Label><Form.Control required type="date" max={today()} value={date} onChange={(event) => setDate(event.target.value)} /><Form.Text>Today or an earlier day only.</Form.Text></Col><Col lg={4} md={8}><Form.Label>Employee</Form.Label><Form.Select required value={mark.employee} onChange={(event) => setMark({ ...mark, employee: event.target.value })}><option value="">Select employee</option>{employees.map((employee) => <option key={employee._id} value={employee._id}>{employee.user.name} · {employee.department?.name}</option>)}</Form.Select></Col><Col lg={2} md={4}><Form.Label>Status</Form.Label><Form.Select value={mark.status} onChange={(event) => setMark({ ...mark, status: event.target.value as Mark['status'] })}><option value="present">Present</option><option value="absent">Absent</option><option value="half-day">Half-day</option><option value="late">Late</option></Form.Select></Col><Col lg={2} md={4}><Form.Label>Punch in</Form.Label><Form.Control type="time" value={mark.checkIn} onChange={(event) => setMark({ ...mark, checkIn: event.target.value })} /></Col><Col lg={2} md={4}><Form.Label>Punch out</Form.Label><Form.Control type="time" value={mark.checkOut} onChange={(event) => setMark({ ...mark, checkOut: event.target.value })} /></Col><Col lg={9}><Form.Label>Remark <span className="text-muted">(optional)</span></Form.Label><Form.Control value={mark.regularizationReason} onChange={(event) => setMark({ ...mark, regularizationReason: event.target.value })} placeholder="Add a note when needed" /></Col><Col lg={3} className="d-flex align-items-end"><Button type="submit" className="w-100" disabled={saving}>{saving ? 'Saving…' : 'Save attendance'}</Button></Col></Row></Form></CardBody></Card><Card className="mb-4"><CardBody><div className="d-flex justify-content-between align-items-center mb-3"><div><h4 className="card-title mb-1">Attendance for {new Date(`${date}T00:00:00`).toLocaleDateString()}</h4><p className="text-muted mb-0">Hours, late arrivals, short hours, and half-day salary deductions are shown below.</p></div><Badge bg="light" text="dark">{records.length} records</Badge></div><ReactTable columns={columns} data={records} pageSize={25} tableClass="text-nowrap mb-0" theadClass="bg-light bg-opacity-50" /></CardBody></Card><AttendanceReportCard /></>
}
