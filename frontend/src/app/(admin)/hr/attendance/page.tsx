...
const AttendancePage = () => {
  ...
  const columns = useMemo<ColumnDef<Record>[]>(() => [
    ...
    { header: 'Correction', cell: ({ row }) => { const request = row.original.correctionRequest; return request ? <div><Badge bg={request.status === 'pending' ? 'warning' : request.status === 'approved' ? 'success' : 'danger'} text={request.status === 'pending' ? 'dark' : undefined}>{request.status}</Badge><small className="d-block text-muted">Requested: {request.requestedStatus}{request.requestedCheckIn ? ` · ${request.requestedCheckIn}` : ''}{request.requestedCheckOut ? `–${request.requestedCheckOut}` : ''}<br />{request.reason}</small>{request.status === 'pending' && <div className="d-flex gap-1 mt-1"><Button size="sm" disabled={saving} onClick={() => decideCorrection(row.original, 'approved')}>Approve & correct</Button><Button size="sm" variant="outline-danger" disabled={saving} onClick={() => decideCorrection(row.original, 'rejected')}>Reject</Button></div>}</div> : '—' } },
