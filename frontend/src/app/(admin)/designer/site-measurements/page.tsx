...
const SiteMeasurementsPage = () => {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError('')
    try {
      const [attachment] = await uploadMultipartFiles<Attachment>([file], token || '', setUploadProgress, '/designer/uploads')
      await apiFetch('/designer/site-measurements', { method: 'POST', body: JSON.stringify({ client: createClient, clientSite: createSite, title, description, attachment }) })
