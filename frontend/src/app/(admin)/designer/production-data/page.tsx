...
const ProductionDataPage = () => {
  ...
  const canViewAll = hasFullAppAccess(user) || canReviewDesignerDocuments(user)
  ...
  return <>
    ...
    <Card><CardBody><div className="row g-2 mb-3"><div className="col-md-6"><Form.Label className="visually-hidden" htmlFor="production-filter-client">Client</Form.Label><Form.Select id="production-filter-client" value={filterClient} onChange={(event) => { setFilterClient(event.target.value); setFilterSite('') }}><option value="">All clients</option>{clients.map((client) => <option key={client._id} value={client._id}>{client.name}</option>)}</Form.Select></div><div className="col-md-6"><Form.Label className="visually-hidden" htmlFor="production-filter-site">Client address</Form.Label><Form.Select id="production-filter-site" disabled={!filterClient} value={filterSite} onChange={(event) => setFilterSite(event.target.value)}><option value="">All addresses</option>{filterSites.map((site) => <option key={site._id} value={site._id}>{site.siteName || site.name} — {site.siteAddress}</option>)}</Form.Select></div></div><div className="d-flex justify-content-between align-items-center mb-3"><h5 className="mb-0">{canViewAll ? 'All production data' : 'My production data'}</h5><Button size="sm" variant="outline-primary" onClick={load} disabled={saving}>Apply filters</Button></div><Table responsive hover className="align-middle mb-0" style={{ minWidth: canViewAll ? 980 : 840 }}><thead><tr><th>Production data</th><th>Client / site</th>{canViewAll && <th>Designer</th>}<th>Created</th><th /></tr></thead><tbody>{records.map((record) => <Fragment key={record._id}><tr><td><div className="fw-semibold">{record.title}</div><small className="text-muted">{record.attachments.length} file{record.attachments.length === 1 ? '' : 's'}</small></td><td><div>{record.client.name}</div><small className="text-muted">{record.clientSite.siteName || record.clientSite.name}{record.clientSite.siteAddress && ` — ${record.clientSite.siteAddress}`}</small></td>{canViewAll && <td>{record.createdBy.name}</td>}<td>{new Date(record.createdAt).toLocaleDateString()}</td><td className="text-end"><Button size="sm" variant="outline-primary" onClick={() => setExpanded(expanded === record._id ? '' : record._id)}>{expanded === record._id ? 'Hide details' : 'View files'}</Button></td></tr>{expanded === record._id && <tr><td colSpan={canViewAll ? 5 : 4} className="bg-body-tertiary"><div className="py-2"><strong>Description</strong><p className="mt-1">{descriptionText(record.description) || 'No description provided.'}</p><strong>Files</strong>{record.attachments.map((attachment) => <a className="d-block mt-1" href={attachment.url} target="_blank" rel="noreferrer" key={attachment.key}>{attachment.originalName || 'Production file'}</a>)}</div></td></tr>}</Fragment>)}{!records.length && <tr><td className="text-center text-muted py-4" colSpan={canViewAll ? 5 : 4}>No production data found.</td></tr>}</tbody></Table></CardBody></Card>
  </>
}
