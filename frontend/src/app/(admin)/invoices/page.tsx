...
const InvoicesPage = () => {
  ...
  useEffect(() => { apiFetch<{ data: Client[] }>('/invoices/client-options').then(({ data }) => setClients(data)).catch((reason) => setError(reason instanceof Error ? reason.message : 'Unable to load clients')) }, [])
  useEffect(() => { load() }, [search, client, status])
  const canCreate = canManageModule(user, 'invoices')
  return <><PageMetaData title="Invoices" /><Card className="mb-3"><CardBody><div className="d-flex justify-content-between align-items-start flex-wrap gap-3"><div><h4 className="card-title mb-1">Invoices</h4><p className="text-muted mb-0">Filter invoices by number, client, or payment status.</p></div>{canCreate && <Link to="/invoices/create"><Button>Create invoice</Button></Link>}</div>{error && <Alert className="mt-3 mb-0" variant="danger">{error}</Alert>}</CardBody></Card><Card><CardBody><div className="row g-2 mb-3"><div className="col-md-4"><Form.Control placeholder="Search invoice number" value={search} onChange={(event) => setSearch(event.target.value)} /></div><div className="col-md-4"><Form.Select value={client} onChange={(event) => setClient(event.target.value)}><option value="">All clients</option>{clients.map((entry) => <option key={entry._id} value={entry._id}>{entry.name}</option>)}</Form.Select></div><div className="col-md-4"><Form.Select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All payment statuses</option><option value="unpaid">Unpaid</option><option value="partially paid">Partially paid</option><option value="paid">Paid</option></Form.Select></div></div><Table responsive hover className="mb-0"><thead><tr><th>Invoice</th><th>Client</th><th>Date</th><th>Grand total</th><th>Status</th><th /></tr></thead><tbody>{invoices.map((invoice) => <tr key={invoice._id}><td>{invoice.invoiceNumber}</td><td>{invoice.client?.name || '-'}</td><td>{new Date(invoice.invoiceDate).toLocaleDateString()}</td><td>{invoice.grandTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td><td><Badge bg={invoice.status === 'paid' ? 'success' : invoice.status === 'partially paid' ? 'warning' : 'secondary'}>{invoice.status}</Badge></td><td className="text-end"><Link to={`/invoices/${invoice._id}`}><Button size="sm" variant="outline-primary" className="me-2">View</Button></Link><PdfActionButton size="sm" variant="outline-secondary" action={() => printPdf(`/invoices/${invoice._id}/pdf`, token).catch((reason) => setError(reason instanceof Error ? reason.message : 'Unable to open invoice PDF'))}>Print PDF</PdfActionButton></td></tr>)}{!invoices.length && <tr><td colSpan={6} className="text-center text-muted py-4">No invoices found.</td></tr>}</tbody></Table></CardBody></Card></>
}
