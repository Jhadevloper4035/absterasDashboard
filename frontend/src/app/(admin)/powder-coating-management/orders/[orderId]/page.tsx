...
export default function PowderCoatingSiteDispatchPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      await apiFetch(`/powder-coating-management/orders/${orderId}/dispatch-to-site`, { method: 'POST', body: JSON.stringify({ challanDate, transportType, vehicleNumber, eWayBillNumber, transportationCost, items: lines.map((line) => ({ ...line, quantity: Number(line.quantity) })) }) })
  ...
  return <>
    ...
    {!order ? <div className="text-center py-5"><Spinner /></div> : <Card><CardBody><Form onSubmit={submit}>
      <div className="row g-3 mb-4">
        <div className="col-md-4"><Form.Label>Order</Form.Label><Form.Control readOnly value={order.orderNo} /></div>
        <div className="col-md-4"><Form.Label>Client</Form.Label><Form.Control readOnly value={order.clientName} /></div>
        <div className="col-md-4"><Form.Label>Client site address</Form.Label><Form.Control readOnly value={order.clientSiteAddressSnapshot || order.clientSiteName || 'Client address'} /></div>
        <div className="col-md-3"><Form.Label>Challan date</Form.Label><Form.Control required type="date" value={challanDate} onChange={(event) => setChallanDate(event.target.value)} /></div>
        <div className="col-md-3"><Form.Label>Transport type</Form.Label><Form.Control value={transportType} onChange={(event) => setTransportType(event.target.value)} /></div>
        <div className="col-md-3"><Form.Label>Vehicle number</Form.Label><Form.Control value={vehicleNumber} onChange={(event) => setVehicleNumber(event.target.value)} /></div>
        <div className="col-md-3"><Form.Label>E-way bill number</Form.Label><Form.Control value={eWayBillNumber} onChange={(event) => setEWayBillNumber(event.target.value)} /></div>
