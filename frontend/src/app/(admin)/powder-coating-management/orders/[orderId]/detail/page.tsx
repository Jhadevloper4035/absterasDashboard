...
type Challan = { _id: string; challanNo: string; type: 'OUT' | 'IN' | 'SITE_OUT'; transportationCost?: number; transportationPaymentScreenshot?: Attachment }
type Order = { _id: string; orderNo: string; clientName: string; clientSiteName?: string; clientSiteAddressSnapshot?: string; vendorName: string; transportationCost?: number; status: 'OUT' | 'RETURNED'; totalQuantity: number; readyQuantity: number; pendingCoatingQuantity: number; deliveredQuantity: number; remainingQuantity: number; deliveryStatus: string; items: Item[]; coatingBatches?: Batch[]; referenceAttachments?: Attachment[]; challans?: Challan[] }
...
export default function PowderCoatingOrderDetailPage() {
  ...
  const canManage = canManageModule(user, 'powder-coating')
  ...
  return <><PageMetaData title="Powder Coating Order" />
    ...
    {loading || !order ? <div className="text-center py-5"><Spinner /></div> : <>
      <Card className="mb-3"><CardBody><div className="row g-3"><div className="col-6 col-md"><small className="text-muted d-block">Planned for coating</small><strong>{order.totalQuantity}</strong></div><div className="col-6 col-md"><small className="text-muted d-block">Coating ready</small><strong className="text-success">{order.readyQuantity}</strong></div><div className="col-6 col-md"><small className="text-muted d-block">Delivered to client</small><strong className="text-success">{order.deliveredQuantity}</strong></div><div className="col-6 col-md"><small className="text-muted d-block">Ready to deliver</small><strong className="text-primary">{order.remainingQuantity}</strong></div><div className="col-6 col-md"><small className="text-muted d-block">Pending coating</small><strong className="text-warning">{order.pendingCoatingQuantity}</strong></div>{Number(order.transportationCost) > 0 && <div className="col-6 col-md"><small className="text-muted d-block">Outward transportation cost</small><strong>₹{Number(order.transportationCost).toFixed(2)}</strong></div>}</div></CardBody></Card>
      ...
      {!!order.challans?.length && <Card className="mb-3"><CardBody><h5 className="mb-1">Challan transportation payments</h5><p className="text-muted mb-3">Add the final transportation payment and proof after the actual cost is known.</p>{order.challans.map((challan) => <div className="border rounded p-3 mb-3" key={challan._id}><div className="fw-semibold">{challan.challanNo} · {challan.type.replace('_', ' ')}</div>{canManage && <TransportationPaymentForm cost={challan.transportationCost} screenshot={challan.transportationPaymentScreenshot} uploadPath="/powder-coating-management/uploads" updatePath={`/powder-coating-management/challans/${challan._id}/transportation-payment`} onSaved={() => void load()} />}</div>)}</CardBody></Card>}
