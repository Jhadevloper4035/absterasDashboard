...
type LaserCutStock = { _id: string; inventoryItemRef: string; itemName: string; vendorRef: string; vendorName: string; quantityAvailable: number; unit?: string; dimensions?: { heightFt?: number; widthFt?: number; lengthFt?: number } }
type LaserCutTransferItem = { laserCutStockRef?: string; inventoryItemRef: string; vendorRef: string; quantity: number }
type LaserCutOrder = { _id: string; orderName: string; customerRef?: string; challans: { type: 'OUT' | 'IN'; status: string; vendorRef: string; vendorName?: string; vendorAddressSnapshot?: string; deliveryAddress?: string; clientSiteRef?: string; clientSiteAddressSnapshot?: string; transportType?: string; vehicleNumber?: string; eWayBillNumber?: string }[] }
...
const materialSize = (dimensions?: LaserCutStock['dimensions']) => dimensions?.heightFt && dimensions.widthFt ? `${dimensions.heightFt} × ${dimensions.widthFt} ft` : dimensions?.lengthFt ? `${dimensions.lengthFt} ft` : '—'
...
export default function CreatePowderCoatingOrderPage({ laserCutOrderId: providedLaserCutOrderId, cancelTo = '/powder-coating-management' }: CreatePowderCoatingOrderProps = {}) {
  ...
  useEffect(() => {
    ...
    if (laserCutOrderId) requests.push(apiFetch<{ data: LaserCutOrder }>(`/laser-cut-management/orders/${laserCutOrderId}`), apiFetch<{ data: LaserCutStock[] }>('/laser-cut-management/stock'), apiFetch<{ data: LaserCutTransferItem[] }>(`/powder-coating-management/laser-cut-orders/${laserCutOrderId}/transferable-items`))
    Promise.all(requests).then((responses) => {
      const [productResponse, vendorResponse, clientResponse] = responses as [{ data: Product[] }, { data: Supplier[] }, { data: Client[] }, { data: LaserCutOrder }?, { data: LaserCutStock[] }?, { data: LaserCutTransferItem[] }?]
      ...
      if (laserCutOrderId) {
        ...
        const transfers = responses[5] as { data: LaserCutTransferItem[] }
        const latestDispatch = order.data.challans.find((challan) => challan.type === 'OUT' && challan.status === 'DISPATCHED')
        const availableLaserStock = transfers.data.flatMap((transfer) => {
          const sourceStock = stock.data.find((entry) => transfer.laserCutStockRef ? entry._id === transfer.laserCutStockRef : entry.vendorRef === transfer.vendorRef && entry.inventoryItemRef === transfer.inventoryItemRef)
          if (!sourceStock) return []
          const quantityAvailable = Math.min(sourceStock.quantityAvailable, transfer.quantity)
          return quantityAvailable > 0 ? [{ ...sourceStock, quantityAvailable }] : []
        })
        ...
        setLines(availableLaserStock.map((entry) => ({ ...blankLine(), source: 'LASER_CUT', laserCutStockRef: entry._id, inventoryItemRef: entry.inventoryItemRef, quantity: String(entry.quantityAvailable) })))
      }
  ...
  useEffect(() => {
    ...
    let active = true
    apiFetch<{ data: Client[] }>(`/clients?parentClient=${clientRef}&limit=100`)
      .then((response) => { if (active) setSites(response.data) })
      .catch((reason) => { if (active) setError(reason instanceof Error ? reason.message : 'Unable to load child client sites') })
    return () => { active = false }
  }, [clientRef])

  const selectedSite = sites.find((site) => site._id === clientSiteRef)
  const laserCutDispatch = laserCutOrder?.challans.find((challan) => challan.type === 'OUT' && challan.status === 'DISPATCHED')
  const childClientAddress = selectedSite?.siteAddress || selectedSite?.shippingAddress || selectedSite?.billingAddress || laserCutDispatch?.clientSiteAddressSnapshot || ''
  ...
  const pickupLocations = [...new Set(lines.map((line) => {
    if (line.source !== 'LASER_CUT') return ''
    const stock = laserStockFor(line.laserCutStockRef)
    const sourceChallan = laserCutOrder?.challans.find((challan) => challan.type === 'OUT' && challan.status === 'DISPATCHED' && challan.vendorRef === stock?.vendorRef)
    const address = sourceChallan?.deliveryAddress || sourceChallan?.vendorAddressSnapshot
    return address ? `${stock?.vendorName || sourceChallan?.vendorName || 'Laser Cut vendor'} · ${address}` : stock?.vendorName || ''
  }).filter(Boolean))]
  ...
  const save = async (event: FormEvent) => {
    ...
    if (!clientRef || !vendorRef) return setError('Select a parent client and powder-coating vendor before creating the challan')
    if (lines.some((line) => {
      const product = productFor(line.inventoryItemRef)
      const shadeName = line.shadeName || product?.shadeName
      const shadeCode = line.shadeCode || product?.shadeCode
      return !line.inventoryItemRef || !Number(line.quantity) || Number(line.quantity) <= 0 || !String(shadeName || '').trim() || !String(shadeCode || '').trim()
    })) {
      return setError('Every product needs a quantity, shade name, and shade code before creating the challan')
    }
      ...
      <Card><CardBody>
        <Form noValidate onSubmit={save}>
          ...
          <Row className="g-3">
            ...
            {laserCutOrder && <><Col md={4}><Form.Label>Laser Cut pickup order</Form.Label><Form.Control readOnly value={laserCutOrder.orderName} /></Col><Col md={4}><Form.Label>Pickup location</Form.Label><Form.Control readOnly value={pickupLocation} placeholder="Select a Laser Cut product below" /></Col><Col md={6}><Form.Label>Powder-coating vendor</Form.Label><Form.Select required value={vendorRef} onChange={(event) => setVendorRef(event.target.value)}><option value="">Select vendor</option>{vendors.map((vendor) => <option key={vendor._id} value={vendor._id}>{vendor.name}</option>)}</Form.Select></Col><Col md={6}><Form.Label>Drop location</Form.Label><Form.Control readOnly value={selectedVendor?.address || ''} /></Col></>}
            ...
            <Col md={12}><Form.Label>Child client address</Form.Label><Form.Control readOnly value={childClientAddress} /></Col>
            {!laserCutOrder && <><Col md={6}><Form.Label>Powder-coating vendor</Form.Label><Form.Select required value={vendorRef} onChange={(event) => setVendorRef(event.target.value)}><option value="">Select vendor</option>{vendors.map((vendor) => <option key={vendor._id} value={vendor._id}>{vendor.name}</option>)}</Form.Select></Col><Col md={6}><Form.Label>Powder-coating vendor address</Form.Label><Form.Control readOnly value={selectedVendor?.address || ''} /></Col></>}
